...
```bash
conda create python=3.8 -n assa
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///assa` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE assa WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE assa TO ;
```
...
ERROR:  database "assa" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists assa
